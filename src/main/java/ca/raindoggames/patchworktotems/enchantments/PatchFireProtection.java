...
public class PatchFireProtection extends Enchantment {
	public PatchFireProtection() {
	...
	public int getDamageProtection(int damage, DamageSource source) {
	    ...
	    return source.isFire() ? damage * 2 : 0;
	}
	
	public static int getFireAfterDampener(LivingEntity entity, int fire) {
	   int i = EnchantmentHelper.getEnchantmentLevel(ModEnchantments.PATCH_PROTECTION_FIRE.get(), entity);
	   if (i > 0) {
	      fire -= Mth.floor((float)fire * (float)i * 0.15F);
	   }
